...
export const Battle = () => {
  ...
  const [problemDescription,setProblemDescription] = useState<string>("Problem Description");
  // Opponent state
  ...
  useEffect(() => {
    ...
    if (oid && oid !== "local-battle") {
      fetchFileContent(oid).then(res => {
         setProblemName(res.file.name);
         setProblemDescription(res.file.problem_definition);
         
         const jsSnippet = res.file.code_snippets?.find((s: any) => s.language === "javascript");
...
const handleRunCode = async() => {
    ...
    try {
        ...
        if(!hasError){
            ...
            socket?.emit("battle_action", { roomId, status: "All Tests Passed! 🏆", progress: 100, result: "OPPONENT_WON" });
            setBattleResult("WON");
        } else {
            // Failure! Broadcast the loss
            socket?.emit("battle_action", { roomId, status: "Execution Failed ❌", progress: 20 });
           alert("Execution Error:\n" + hasError);
        }
    } catch (error) {
        console.error("Execution failed", error);
       socket?.emit("battle_action", { roomId, status: "Error ❌", progress: 0 });
    }
} 
      ...
      {battleResult && (
        <div className="absolute inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm">
          <div className="flex flex-col items-center justify-center p-12 bg-[#0b0c0e] border border-white/10 rounded-2xl shadow-2xl max-w-md w-full text-center relative overflow-hidden">
            ...
            <div className={`absolute top-0 left-1/2 -translate-x-1/2 w-32 h-32 blur-[80px] rounded-full ${battleResult === "WON" ? "bg-cyan-500/30" : "bg-rose-500/30"}`} />
