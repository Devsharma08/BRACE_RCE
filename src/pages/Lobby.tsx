...
const Lobby = () => {
  ...
  const renderCard = (room: Room, isArchive: boolean) => {
    ...
    return (
      <article
        key={room.id}
        className={`border border-white/6 ${accentBorder} border-t-2 bg-[#0c0f18] p-5 flex flex-col gap-3`}
      >
        <div className="flex items-start justify-between gap-2">
          ...
          {!room.isPublic && (
            <Lock
              className="w-3.5 h-3.5 text-[#8892A4] shrink-0 mt-0.5"
        ...
        <div className="flex flex-wrap items-center gap-2 text-xs text-[#8892A4]">
        ...
        <div className="flex items-center justify-between mt-auto pt-3 border-t border-white/6">
          <div className="flex items-center gap-2 min-w-0">
            <div
              className="w-6 h-6 shrink-0 bg-[#111520] border border-cyan-500/15 flex items-center justify-center"
              aria-hidden
            >
            ...
            <span
              className="text-[10px] text-[#8892A4] truncate"
          ...
          {isArchive ? (
            <button
              ...
              className="shrink-0 text-[10px] text-[#8892A4] hover:text-[#FF3B5C] transition-colors flex items-center gap-1 px-2 py-1"
            ...
            <button
              ...
              className="shrink-0 text-[10px] text-cyan-400 border border-cyan-500/30 hover:border-cyan-500 hover:bg-cyan-500/10 px-3 py-1.5 transition-all flex items-center gap-1.5"
  ...
  return (
    <div className="flex min-h-screen bg-[#050608] text-slate-100 font-mono">
      ...
      >
        {/* Dot-grid texture */}
        <div className="fixed inset-0 pointer-events-none opacity-[0.03] bg-[radial-gradient(rgba(0,243,255,0.04)_1px,transparent_1px)] [background-size:48px_48px] -z-10" />
        ...
        <div
          className="mb-8 relative flex items-center justify-center h-48 border border-white/6 bg-[#0c0f18] overflow-hidden"
          aria-hidden="true"
        >
          <div className="absolute inset-0 pointer-events-none opacity-[0.035] bg-[radial-gradient(rgba(0,243,255,0.04)_1px,transparent_1px)] [background-size:48px_48px]" />
          ...
          <div className="absolute bottom-4 left-4 text-[10px] text-[#8892A4] font-mono">
            SCANNING...
          </div>
        ...
        <div
          ...
          className="flex items-center border-b border-white/6 mb-6 overflow-x-auto"
        >
          {(["ROOMS", "TEMPLATES", "MY_ARCHIVES"] as const).map(tab => (
            <button
              ...
              className={`px-4 sm:px-5 py-3 text-xs font-medium uppercase tracking-wide transition-all whitespace-nowrap ${
                activeTab === tab
                  ? "border-b-2 border-[#00D4FF] text-[#00D4FF] bg-cyan-500/5"
